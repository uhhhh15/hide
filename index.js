...
function updateTokenStatsUI() {
    ...
    // 添加一个新函数来查找以指定前缀开头的节点（用于匹配 chatHistory-6, chatHistory-7 等）
    const findCollectionByPrefix = (c, prefix) => {
        if (c.identifier && c.identifier.startsWith(prefix)) return c;
        if (c.collection) {
            for (const i of c.collection) {
                if (i instanceof Object && i.collection) {
                    const f = findCollectionByPrefix(i, prefix);
                    if (f) return f;
                }
            }
        }
    ...
    const chatHistory = findCollectionByPrefix(pm.messages, 'chatHistory');
